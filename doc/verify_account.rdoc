...
after verifying the account, unless +verify_account_grace_period+ is set.
Depends on the login and create account features.

== Auth Value Methods

account_created_at_column :: The column in the accounts table that holds the created at timestamp,
                             used if +verify_account_grace_period+ is set.
...
unverified_account_session_key :: The session key set if the logged in account has not been
                                  unverified.
...
verify_account_grace_period :: The amount of seconds after an account creation that a user
                               will be able to login without verifying.  If this is set, Rodauth
                               defaults to logging in users upon account creation.
...
account_created_at :: The timestamp of the current account's creation.
account_from_verify_account_key(key) :: Retrieve the account using the given verify
                                        account key, or return nil if no account
                                        matches.
account_in_unverified_grace_period? :: Whether the current account is in an unverified
                                       grace period.
