...
SMS messages.  It is usually used as a backup if other two factor authentication is not available
...
sms_auth_link_text :: The text to use for the link from the two factor auth page.
...
sms_codes_primary? :: Whether SMS codes are a primary second factor.  If not, they cannot be setup unless another second factor is already present.
...
sms_disable_link_text :: The text to use for the remove link from the two factor manage page.
...
sms_setup_link_text :: The text to use for the setup link from the two factor manage page.
