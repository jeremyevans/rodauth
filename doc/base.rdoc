...
return_to_path_max_size :: The maximum path size in bytes to allow when returning to requested location (login, two_factor_base, confirm_password), 2048 by default to avoid exceeding the 4K cookie size limit
...
transaction_opts :: Any options to pass when running a transaction. Can be used to set transaction isolation levels, among other things.
...
account_reload(old_type, new_type) :: What to do when loading an account when one is already loaded. The arguments are the retrieval types. By default, does nothing for non-rodauth routes, and for rodauth routes, warns if old_type and new_type are the same (this indicates unnecessary reloading), and raises if they are different (to avoid potential account confusion).
...
clear_session :: Clears the current session. For Roda's session plugin uses +clear_session+ on the Roda scope. Otherwise, does +session.clear+. If you are using a session implementation based on Rack::Session::Abstract::Persisted, first consider an alternative, and if you decide not to use an alternative you should set this to <tt>session.options[:renew] = true; super()</tt> or you can be vulnerable to session fixation.
missing_recommended_configuration(msg) :: The action to take if the Rodauth configuration does not include recommended configuration methods, warns by default.
...
valid_return_to_path?(path) :: Whether the path given is a valid return to path. By default, must start with <tt>/</tt>, cannot start with <tt>//</tt>, and must be under +return_to_path_max_size+ bytes.
