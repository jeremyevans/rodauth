...
When refreshing the access token, it is best to provide an access token, but it is not
required.  However, if you do not provide an access token and only provide the refresh
token, then you will lose the contents of the current session, such as how the current
session was originally authenticated.
