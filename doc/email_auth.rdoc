...
email_auth_request_error_flash :: The flash error to show if not able to send a email auth email.
...
before_email_auth_request_route :: Run arbitrary code before handling a email auth request route.
before_email_auth_route :: Run arbitrary code before handling a email auth route.
...
email_auth_request_form :: The HTML to use for a form to request a email auth email, shown on the login page after the user submits their login, if +force_email_auth?+ is false.
