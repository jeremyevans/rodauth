...
jwt_cors_allow_origin :: Which origins are allowed to perform CORS requests.  The default is +false+.  This can be a String, Array of Strings, Regexp, or +true+ to allow CORS requests from any domain. If providing a Regexp, ensure that it uses <tt>\A</tt> and <tt>\z</tt> appropriately to anchor the pattern.
...
jwt_cors_vary_header :: For allowed CORS requests, the value returned in the Vary header (default: 'Origin'). This instructs caches not to return the value from one origin to another origin.
