...
The webauthn feature implements 2 factor authentication via WebAuthn.
It supports registering WebAuthn authenticators, using them for 2
factor authentication, and removing WebAuthn authenticators.
...
without using javascript.  See Rodauth's tests for how this works.
...
webauthn_auth_button :: Text to use for button on form to authenticate via WebAuthn.
...
webauthn_extensions :: The value of the WebAuthn authenticatorSelection option when registering a new WebAuthn authenticator or authenticating via WebAuthn.
...
webauthn_remove_button :: Text to use for button on form to remove an existing WebAuthn authenticator.
webauthn_remove_error_flash :: The flash error to show if unable to remove an existing WebAuthn authenticator.
webauthn_remove_link_text :: The text to use for the remove link from the two factor manage page.
...
webauthn_setup_button :: Text to use for button on form to register a new WebAuthn authenticator.
...
webauthn_setup_link_text :: The text to use for the setup link from the two factor manage page.
...
valid_new_webauthn_credential?(webauthn_credential) :: Check wheck the WebAuthn new credential registration provided by the client is valid.
valid_webauthn_credential_auth?(webauthn_credential) :: Check wheck the WebAuthn authentication provided by the client is valid.
...
webauthn_user_name :: The user name to use when registering a new WebAuthn credential.
