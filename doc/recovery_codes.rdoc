...
codes.  It is usually used as a backup if other two factor authentication methods are
not available or has been locked out, but can be used by itself.  It allows
...
auto_add_recovery_codes? :: Whether to automatically add recovery codes (or any missing recovery codes) when another 2nd factor is enabled (false by default).
...
recovery_auth_link_text :: The text to use for the link from the two factor auth page.
...
recovery_codes_link_text :: The text to use for the setup link from the two factor manage page.
recovery_codes_param :: The parameter name for the recovery code.
recovery_codes_primary? :: Whether recovery codes are a primary second factor.  If not, they cannot be setup unless another second factor is already present.
