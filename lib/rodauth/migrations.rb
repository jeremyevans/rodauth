...
module Rodauth
  def self.create_database_authentication_functions(db, opts={})
    ...
    when :postgres
      ...
      db.run <<~SQL
        CREATE OR REPLACE FUNCTION #{get_salt_name}(acct_id #{primary_key_type}, type text) RETURNS text AS $$
        DECLARE salt text;
        BEGIN
        SELECT
          CASE
            WHEN type = 'argon2' AND password_hash ~ '^\\$argon2id'
    ...
    when :mysql
      db.run <<~SQL
        CREATE FUNCTION #{get_salt_name}(acct_id int8, type varchar(255)) RETURNS varchar(255)
    ...
    when :mssql
      db.run <<~SQL
        CREATE FUNCTION #{get_salt_name}(@account_id bigint, @type nvarchar(255)) RETURNS nvarchar(255)
        ...
        SELECT @salt =
          CASE
            WHEN @type = 'argon2' AND password_hash LIKE '[$]argon2id%'
