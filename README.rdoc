...
=== Account Verification/Password Resets/Remember Me

Account verification, password resets, and remember features all use a
similar approach.  They both provide a token, in the format
"id_long-random-string".  By including the id of the account in the
token, an attacker can only attempt to bruteforce the token for a
single account, instead of being able to bruteforce tokens for all
accounts at once (which would be possible if the token was just a random
string).

There is only a single active account verification/password reset/remember
token for an account at a time.  This prevents attackers from creating
