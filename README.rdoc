...
needs to be enabled in addition to the login feature. If you want to
force all users to use OTP authentication, requiring users that don't
currently have an account to set one up:
  ...
  route do |r|
    r.rodauth
    rodauth.require_login
    rodauth.require_otp_authenticated

    # ...
  end

If you want to support but not require 2 factor authentication, you
you only redirect if the user has setup OTP: 

  route do |r|
    r.rodauth
    rodauth.require_login
    rodauth.require_otp_authenticated if rodauth.has_otp?

    # ...
  end

Similarly to requiring authentication in general, it's possible to require
2 factor authentication only for particular branches:
