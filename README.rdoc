...
=== With External Session Authentication (such as facebook (yuck!))

You can authenticate using a session from another service that allows it, such
as facebook (yuck!), by overriding a couple of methods:

   require 'koala'
   plugin :rodauth, :json=>:only, :name=>:facebook do
    enable :login, :jwt
    jwt_secret File.read('.jwt_secret')

    login_param 'access_key'

    account_from_login do |login|
      fb = Koala::Facebook::API.new(login)
      me = fb.get_object('me', :fields=>%i(email))

      return nil unless me
      return nil unless me['email']
...
and mounting on a separate path:
...
This should allow you to post your +access_token+ to +/facebook/login+ and get
back a rodauth session, which can be treated as usual.
